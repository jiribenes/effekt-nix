  ...
  outputs = { self, nixpkgs, flake-utils, sbt-derivation }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        effektBackends = {
          ...
          js-web = {
            ...
            outputName = "js-web";
            buildInputs =
              [ pkgs.nodejs ]; # For tests, we currently use the 'js' backend
          ...
          llvm = {
            ...
            outputName = "llvm";
            buildInputs =
              [ pkgs.llvm pkgs.clang pkgs.libuv ]; # Needed for compilation
        ...
        buildEffektRelease =
          ...
          pkgs.stdenv.mkDerivation {
            ...
            src = pkgs.fetchurl {
              url =
                "https://github.com/effekt-lang/effekt/releases/download/v${version}/effekt.tgz";
              inherit sha256;
            };
        ...
        buildEffektFromSource = { src, version, depsSha256
          , # SHA256 of the Scala dependencies
          ...
          sbt-derivation.lib.mkSbtDerivation {
            ...
            nativeBuildInputs =
              [ pkgs.nodejs pkgs.maven pkgs.makeWrapper pkgs.gnused ];
            buildInputs = [ pkgs.jre ]
              ++ pkgs.lib.concatMap (b: b.buildInputs) backends;
        ...
        buildEffektPackage = { pname, # package name
          ...
          in pkgs.stdenv.mkDerivation {
            ...
            nativeBuildInputs = [ effektBuild pkgs.gnused pkgs.makeWrapper ]
              ++ pkgs.lib.concatMap (b: b.buildInputs) backends;
              ...
              ${pkgs.lib.concatMapStrings (backend:
                ...
                || (backend.runtimeInputs != [ ]) then ''
                  ...
                  makeWrapper $out/bin/${pname}-${backend.outputName}.unwrapped $out/bin/${pname}-${backend.outputName} \
                    --prefix PATH : ${
                      pkgs.lib.makeBinPath backend.runtimeInputs
                    }
                '' else
                  "") backends}
            ...
            checkPhase = pkgs.lib.concatMapStrings (test:
              pkgs.lib.concatMapStrings (backend:
                let
                  backendForCheck = if backend == effektBackends.js-web then
                    effektBackends.js
                  else
                    backend;
                in ''
                  ...
                  makeWrapper $TMPDIR/testout/$(basename ${test} .effekt).unwrapped $TMPDIR/testout/$(basename ${test} .effekt) \
                    --prefix PATH : ${
                      pkgs.lib.makeBinPath backendForCheck.runtimeInputs
                    }
        ...
        mkDevShell = { effekt ? null, effektVersion ? latestVersion
          , backends ? [ effektBackends.js ] }:
          let
            effektBuild = if effekt != null then
              effekt
        ...
        compilerDevShell = pkgs.mkShell {
          buildInputs = with pkgs;
            [ sbt jre maven scala_3 ] ++ pkgs.lib.concatMap (b: b.buildInputs)
        ...
        autoPackages = pkgs.lib.mapAttrs' (version: _:
          ...
          "effekt_${builtins.replaceStrings [ "." ] [ "_" ] version}"
          (buildEffektRelease {
            inherit version;
        ...
        latestEffekt = autoPackages."effekt_${
            builtins.replaceStrings [ "." ] [ "_" ] latestVersion
          }";

        # Helpful function to get an Effekt package given version and backends
        getEffekt = { version ? null
          , # Version as a string (leave null for the latest version)
          backends ? [ effektBackends.js ] # Supported backends
          }:
          assert backends != [ ]; # Ensure at least one backend is specified
          let
            selectedVersion =
              if version == null then latestVersion else version;
            sha256 = effektVersions.${selectedVersion} or null;
          in if sha256 == null then
            throw "Unsupported Effekt version: ${selectedVersion}"
          else
            buildEffektRelease {
              inherit backends;
      ...
      in {
        ...
        lib = {
          inherit buildEffektRelease buildEffektFromSource buildEffektPackage
            getEffekt mkDevShell effektBackends;
        };

        # Automatically generated packages + latest version (as default)
        packages = autoPackages // { default = latestEffekt; };
