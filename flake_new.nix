  ...
  outputs = { self, nixpkgs, flake-utils, sbt-derivation }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Helper function to create Effekt wrapper that auto-injects libuv paths for LLVM backend
        mkEffektWrapper = backends: let
          hasLLVM = builtins.any (b: b.name == "llvm") backends;
        in if hasLLVM then ''
          # Create a smart wrapper that auto-injects libuv paths for LLVM backend
          cat > $out/bin/effekt << 'EOF'
#!/bin/sh
# Parse arguments to detect LLVM backend usage
use_llvm=false
new_args=""
while [ $# -gt 0 ]; do
  case "$1" in
...
# Execute with or without libuv flags
if [ "$use_llvm" = "true" ]; then
  exec ${pkgs.jre}/bin/java -jar $out/lib/effekt.jar \
    --clang-libraries ${pkgs.libuv}/lib \
    --clang-includes ${pkgs.libuv}/include \
    $new_args
else
  exec ${pkgs.jre}/bin/java -jar $out/lib/effekt.jar $new_args
fi
EOF
          chmod +x $out/bin/effekt
        '' else ''
          # Standard wrapper without libuv injection
        ...
        buildEffektRelease = {
          ...
          pkgs.stdenv.mkDerivation {
            ...
            installPhase = ''
              ...
              ${mkEffektWrapper backends}
              # Add backend tools to PATH if needed
              ${let
                backendTools = pkgs.lib.concatMap (b: b.buildInputs) backends;
              in if backendTools != [] then ''
                mv $out/bin/effekt $out/bin/effekt.unwrapped
                makeWrapper $out/bin/effekt.unwrapped $out/bin/effekt \
                  --prefix PATH : ${pkgs.lib.makeBinPath backendTools}
              '' else ""}
            '';
            meta = effektMeta;
          };
            ...
            pkgs.stdenv.mkDerivation {
              ...
              buildPhase = ''
                ...
                ${pkgs.lib.concatMapStrings (backend: ''
                  ...
                  ${if backend.name == "llvm" then ''
                    # For LLVM backend, pass libuv paths explicitly
                    effekt --build --backend ${backend.name} \
                      --clang-libraries ${pkgs.libuv}/lib \
                      --clang-includes ${pkgs.libuv}/include \
                      ${src}/${main}
                  '' else ''
              ...
              checkPhase = pkgs.lib.concatMapStrings (test:
                pkgs.lib.concatMapStrings (backend:
                  ...
                  in ''
                    ...
                    ${if backendForCheck.name == "llvm" then ''
                      effekt --build --backend ${backendForCheck.name} \
                        --clang-libraries ${pkgs.libuv}/lib \
                        --clang-includes ${pkgs.libuv}/include \
                        --out $TMPDIR/testout ${src}/${test}
                    '' else ''
